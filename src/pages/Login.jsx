...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      // First login with this email auto-registers an account under the
      // name/mobile entered here; logging in again with the same email
      // signs back into that same account and its saved details.
      login({ name, email, mobile, role });
      navigate('/dashboard');
    }, 700);
  };
  ...
  return (
    <div className="min-h-screen w-full bg-secondary-500 relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-5xl grid lg:grid-cols-2 rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
        ...
        <div className="hidden lg:flex flex-col justify-between bg-gradient-to-br from-primary-700 via-primary-600 to-secondary-500 p-10 relative overflow-hidden">
          {/* Ambient rotating sunburst echoing the brand mark */}
          <div className="absolute -right-20 -top-20 h-72 w-72 opacity-[0.12] sunburst-spin pointer-events-none">
            <SunburstGlyph />
          </div>
          ...
          >
            <div className="h-9 w-9 rounded-md overflow-hidden ring-1 ring-white/25 shadow-lg">
        ...
        <div className="bg-white/[0.04] backdrop-blur-2xl p-8 sm:p-12 flex flex-col justify-center border-l border-white/10">
          <div className="lg:hidden flex items-center gap-2.5 mb-8">
            <div className="h-9 w-9 rounded-md overflow-hidden ring-1 ring-white/20">
              <img src={abgLogo} alt="Aditya Birla Group" className="h-full w-full object-cover" />
            </div>
            <span className="font-display font-semibold text-white tracking-wide">SmartPlant 360</span>
          </div>
          ...
          >
            ...
            <div>
              <label className="text-xs font-medium text-white/60 mb-1.5 block">Mobile number</label>
              <input
                type="tel"
                required
                value={mobile}
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="relative overflow-hidden w-full rounded-lg bg-gradient-to-r from-primary-500 to-accent-500 hover:from-primary-400 hover:to-accent-400 transition-all text-white font-medium text-sm py-2.5 mt-2 disabled:opacity-70 shadow-lg shadow-primary-500/20"
            >
          ...
          <p className="text-xs text-white/30 mt-8 text-center">
            Demo build — entering a new email creates an account under your name;
            the same email next time signs you back into it.
          </p>
        </div>
      </div>
    </div>
...
function SunburstGlyph() {
  // Abstract rays echoing the Aditya Birla Group sunburst mark — decorative only.
