  ...
  return (
    ...
    >
      ...
      <div className="mt-1.5 flex items-center gap-1">
        {isPositive ? (
          <ArrowUpRight className="h-3.5 w-3.5 text-primary-500" strokeWidth={2.5} />
        ) : (
          <ArrowDownRight className="h-3.5 w-3.5 text-danger-500" strokeWidth={2.5} />
        )}
        <span className={`text-xs font-medium font-mono-data ${isPositive ? 'text-primary-500' : 'text-danger-500'}`}>
          {isPositive ? '+' : ''}{delta}%
        </span>
